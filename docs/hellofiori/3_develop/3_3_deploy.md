...
1. Build your application in SAP Business Application Studio. 
    ...
    With this, a folder "mta_archives" and an mtar file are created.
...
1. Open your Cloud Foundry extension in BAS and create a Cloud Foundry target.
    ...
    Provide username (e.g., email address) and password or follow the instructions for SSO Passcode.

    ![](images/3_0_1_cf_login_target.png)


2. Alternatively, you can use Command Palette to log in to Cloud Foundry. You will not create a CF target this way.   

    In the menu, click "View" --> "Command Palette".
...
1.  Open the project folder **mta_archives** and right-click on the built file `mta_archives/sapbtphelloworldui5_0.0.1.mtar` and select **Deploy MTA Archive**.
    ...
    The deployment target is the Cloud Foundry subaccount to which you are logged in. 
...
If you have no deployment configuration, perhaps because you forgot to add it during project creation, you can still add it. 

1. Open your Business Application Studio (BAS).

2. Open from the menu "View" the Command Palette (Ctrl+Shift+P).

   Search for the command `Fiori: Add Deployment Configuration`.
